        ...
        >
          ...
          <ul className="p-2">
            {tipsparapintar?.map((item, index) => (
              <li
                className="mt-4 cursor-pointer p-2 hover:bg-white text-black 
                            text-xl transition-transform transform duration-500 hover:scale-105 uppercase"
                key={index}
                onClick={() => handleTips(item.target)}
              >
                {item.name}
              </li>
            ))}
        ...
        >
          {tuoriales?.map((item, index) =>
            item.video ? (
              <Card
                key={index}
                className={`mt-6 ${windowWidth < 860 ? ' w-[70%]' : 'w-[90%]'}`}
              >
                <CardHeader color="blue-gray" className="relative h-[30vh]">
                  <div key={index} className="">
                    <ReactPlayer
                      url={item.video}
                      width={'auto'}
                      height={windowWidth < 1000 ? 200 : 300}
                      loop
                      controls
                      className={'w-full'}
                    />
              ...
              >
                <CardHeader color="blue-gray" className="relative ">
                  <Image
                    ...
                    width={500}
                    height={600}
                    className={`${item.causas ? 'w-[30%]' : 'w-full'}`}
                    alt="vinilicas"
                  ></Image>
                </CardHeader>
                <CardBody>
