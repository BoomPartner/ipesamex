...
const DiscoverColor = () => {
  ...
  const handleDescubreColor = () => {
    ...
    switch (estilo) {
      case 'contemporaneo':
        const filter1 = coloresDiscover.filter(
          (item) => item.estilo == 'contemporaneo'
        );
        if (filter1) {
      ...
      case 'escandinavo':
        const filter2 = coloresDiscover.filter(
          (item) => item.estilo == 'escandinavo'
        );
        if (filter2) {
      ...
      case 'minimalista':
        const filter3 = coloresDiscover.filter(
          (item) => item.estilo == 'minimalista'
        );
        if (filter3) {
      ...
      case 'moderno':
        const filter4 = coloresDiscover.filter(
          (item) => item.estilo == 'moderno'
        );
        if (filter4) {
    ...
    <div className="color-sucursales">
      <section>
        <div className="background-discover bg-black bg-opacity-20 relative">
          <div className="w-full absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
            <Typography
              variant={windowWidth < 1080 ? 'h4' : 'h1'}
              className="text-white text-center uppercase arriba"
            >
              Descubre tu color
            </Typography>
          </div>
      ...
      <section className="w-full">
        <div className="p-10">
          <div className="w-full mt-10">
            <Typography
              as={'h2'}
              variant={windowWidth < 1080 ? 'h5' : 'h2'}
              className="mt-20 text-black text-center upeprcase arriba2 uppercase"
            >
              CONOCE CUÁLES COLORES SE ADAPTAN MÁS A TU ESTILO DE VIDA
            </Typography>
          </div>
          ...
          <div className="w-full mt-24">
            <Typography
              ...
              variant={windowWidth < 1000 ? 'h5' : 'h3'}
            >
            ...
            {espacios?.map((item, index) => (
              <>
                <div
                  ...
                  className={`shadow-xl transform duration-500 hover:scale-110 group ${
                    activeIndex1 === index
                      ? 'scale-110-active'
                      : 'hover:scale-110'
                  }`}
                  ...
                  >
                    <div
                      className={`bg-black bg-opacity-0 ${
                        activeIndex1 === index
                          ? 'bg-opacity-50 text-[#c89919]'
                          : 'hover:bg-opacity-50 hover:text-[#c89919]'
                    ...
                    >
                      <div
                        className={` ${
                          activeIndex1 === index ? 'opacity-80 ' : 'opacity-0 '
                        } h-[45vh] text-white text-center flex justify-center items-center text-2xl group-hover:opacity-80 transform duration-500 uppercase`}
                      >
                        {item.title}
                      </div>
                    </div>
      ...
      <section>
                ...
                >
                  ...
                  >
                    ...
                    >
                      <div
                        className={` ${
                          activeIndex2 === index ? 'opacity-80 ' : 'opacity-0 '
                        } h-[45vh] text-white text-center flex justify-center items-center text-2xl group-hover:opacity-80 transform duration-500 uppercase`}
                      >
                        {item.title}
                      </div>
        ...
        <div className="w-full flex justify-center mt-24">
          ...
          espacio.trim() !== '' ? (
            <div
              className="cursor-pointer agrande bg-[#c50411] text-white p-2 px-4 rounded-lg"
              onClick={handleDescubreColor}
            >
        ...
        {mostrar && (
          <div
            className={`w-full p-10 ${
              windowWidth < 1200 ? 'block' : 'flex justify-center'
            }`}
          >
            <div className={windowWidth < 1200 ? 'mx-auto w-[80%]' : 'w-[60%]'}>
              <div className="w-full">
                ...
                >
                  ...
                  >
                    <div
                      className={`bg-black  bg-opacity-20 text-[#c89919] transform duration-300 cursor-pointer`}
                      style={{
                        objectFit: 'cover',
                        width: '100%',
                        height: '100%',
                    ...
                    >
                      <div
                        className={`opacity-80  h-[45vh] text-white text-center flex justify-center items-center text-2xl group-hover:opacity-80 transform duration-500 uppercase`}
                      >
                        {espacio}
                      </div>
                    </div>
            ...
            <div
              className={`w-full ${
                windowWidth < 1200 ? 'ml-0 mt-24' : 'ml-[5%]'
          ...
          <div className="w-full mt-24 mb-10">
            <div className="flex justify-center">
              <Typography as={'h3'} variant="h3" className="uppercase">
                Prueba tus colores en nuesrto decorador
              </Typography>
            </div>
